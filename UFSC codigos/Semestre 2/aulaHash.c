...
// s = chave para calculo do hash
// TABLE_SIZE = tamanho da tabela
int gera_hash(char* s, int TABLE_SIZE){
   int hash = 0, i;
   int len_s = strlen(s); //tamanho da cadeia de caracteres
   for (i=0;i<len_s;i++){
      hash  = hash + s[i];        
    }
...
void insere(Hash *h, Aluno *a){
   if(h==NULL){
      ...
      */
        int calculoHash = gera_hash(a->nome,h->TABLE_SIZE);
        if (h->itens[calculoHash]==NULL){
            h->itens[calculoHash] = a;
            printf("%d",calculoHash);
        }
...
void remover(Hash *h, char *chave){
   if(h==NULL){
      ...
      int calculoHash = gera_hash(chave,h->TABLE_SIZE);
